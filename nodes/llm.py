    ...
    def INPUT_TYPES(cls) -> InputTypeDict:
        return {
            "required": {
                ...
                "num_predict": (IO.INT, {"default": -1, "min": -1, "max": 2048, "step": 1, "tooltip": "Maximum number of tokens to predict when generating text. -1 means no limit."}),
                "top_k": (IO.INT, {"default": 40, "min": 1, "max": 1000, "step": 1, "tooltip": "Limits the number of tokens to consider for each step."}),
                "top_p": (IO.FLOAT, {"default": 0.9, "min": 0.0, "max": 1.0, "step": 0.01, "tooltip": "Works together with top-k. A higher value (e.g., 0.95) will lead to more diverse text, while a lower value (e.g., 0.5) will generate more focused and conservative text. "}),
                "repeat_last_n": (IO.INT, {"default": 64, "min": 0, "max": 256, "step": 1, "tooltip": "Sets how strongly to penalize repetitions. A higher value (e.g., 1.5) will penalize repetitions more strongly, while a lower value (e.g., 0.9) will be more lenient. "}),
                "temperature": (IO.FLOAT, {"default": 0.8, "min": 0.0, "max": 1.0, "step": 0.01, "tooltip": "The temperature of the model. Increasing the temperature will make the model answer more creatively."}),
                "repeat_penalty": (IO.FLOAT, {"default": 1.1, "min": 1.0, "max": 2.0, "step": 0.01, "tooltip": "Penalizes repeated tokens in the output."}),
...
class Sage_OllamaLLMPromptText(ComfyNodeABC):
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                ...
                "keep_alive": (IO.FLOAT, {"default": 0.0, "min": -1.0, "max": 60.0 * 60.0, "step": 1, "tooltip": "Time in seconds to keep the connection alive. -1 to keep alive indefinitely."})
                },
            "optional": {
                "options": ("OLLAMA_OPTIONS", {"default": {}}),
                "system_prompt": (IO.STRING, {"forceInput": True, "default": "", "multiline": True, "tooltip": "System prompt to guide the model's behavior."})
            }
        }
    ...
    def get_response(self, prompt: str, model: str, seed: int = 0, keep_alive: float = 0.0, options: dict = {}, system_prompt: str = "") -> tuple:
        if options is None:
            options = {}
...
class Sage_OllamaLLMPromptVision(ComfyNodeABC):
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                ...
                "image": (IO.IMAGE, {}),
                "seed": (IO.INT, {"default": 0, "min": 0, "max": 2**32 - 1, "step": 1, "tooltip": "Seed for random number generation."}),
                "keep_alive": (IO.FLOAT, {"default": 0.0, "min": -1.0, "max": 60.0 * 60.0, "step": 0.1, "tooltip": "Time in seconds to keep the connection alive. -1 to keep alive indefinitely."})
            },
            "optional": {
                "options": ("OLLAMA_OPTIONS", {"default": {}}),
                "system_prompt": (IO.STRING, {"forceInput": True, "default": "", "multiline": True, "tooltip": "System prompt to guide the model's behavior. Leave empty to use the model's default behavior."})
            }
        }
    ...
    def get_response(self, prompt: str, model: str, image, seed: int, keep_alive: float = 0.0, options: dict = {}, system_prompt: str = "") -> tuple:
        if options is None:
            options = {}
        if not llm.OLLAMA_AVAILABLE:
            raise ImportError("Ollama is not available. Please install it to use this node.")
