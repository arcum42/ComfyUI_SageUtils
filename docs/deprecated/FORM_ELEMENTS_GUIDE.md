...
The Sage Utils form component documentation has been moved to the OKF bundle.

See `okf/ui/form_guide.md` for the current canonical form guide and API reference.

This file now serves as a backward-compatible pointer to the structured OKF documentation.

> Note: The OKF `ui` subbundle is the primary source for UI documentation. Legacy docs under `docs/` are retained only as redirects and quick references.
